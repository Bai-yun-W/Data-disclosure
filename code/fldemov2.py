...
def save_to_csv(filename, content):
    df = pd.DataFrame(content, columns=["Epoch", "Client", "Loss", "MAE"])
    df.to_csv(filename, mode='a', header=not pd.io.common.file_exists(filename), index=False)

# 主函数
if __name__ == "__main__":
    # 1. 加载数据
    csv_file = 'data/5_mm.csv'  # 请替换为您的 CSV 文件路径
    ...
    for epoch in range(epochs):
        ...
        for client_id in range(num_clients):
            ...
            # 保存每个客户端的训练结果
            save_to_csv('result/training_results_1dataset.csv', [[epoch+1, client_id+1, loss, None]])
        
        # 聚合客户端模型，传递数据大小列表
        aggregate_models(client_models, client_data_sizes)

        # 7. 计算并保存全局模型的MAE
        model = client_models[3]  # 使用第一个客户端的模型作为全局模型
        model.eval()
        predictions = []
        actuals = []
        with torch.no_grad():
        ...
        print(f"Epoch {epoch+1} Global Model MAE: {mae}")

        # 保存全局模型的MAE
        save_to_csv('result/training_results_1dataset.csv', [[epoch+1, "Global Model", None, mae]])
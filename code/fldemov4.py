...
if __name__ == "__main__":
    ...
    # 5. 为每个客户端设置不同的训练轮次
    rounds_per_client = [5, 3, 6, 4, 7, 5, 3, 6, 4, 7]  # 每个客户端的训练轮次
    ...
    for epoch in range(epochs):
        ...
        for client_id in range(num_clients):
            print(f"Training client {client_id+1} with {rounds_per_client[client_id]} rounds...")
            loss = client_train_multiple_rounds(
                ...
                criterion,
                device,
                rounds=rounds_per_client[client_id]  # 根据客户端的具体轮次数量
            )
